...
def del_salesman(request):
    ...
    if request.method == "POST":
        ...
        user_name = body['user_name']
        user = models.User.objects.filter(user_name=user_name)
        user_str = models.UserStr.objects.filter(user_name=user_name)
        user_str.delete()
        user.delete()
        response['status'] = True
        return JsonResponse(data=response, safe=False)
...
def search_with_salesman(request):
    ...
    if projects_id:
        for project_id in projects_id:
            single_project = {}
            project = models.Project.objects.filter(project_id=project_id.project_id, effective=0)[0]
            ...
            users = models.UserProject.objects.filter(project_id=project.project_id)
            peoples = []
            # 循环放入负责人姓名
            for user in users:
...
def single_project_report(request):
    ...
    if request.method == "GET":
        ...
        for report in reports:
            ...
            responses.append(response)
        # 返回汇报列表
        return JsonResponse(data=responses[::-1], safe=False)
...
def add_report(request):
    ...
    if request.method == 'POST':
        ...
        # 插入数据库
        models.Report.objects.create(project_id=project_id, progress=progress, report_name=report_name,
...
def get_ones_record(request):
    if request.method == "GET":
        ...
        # 返回一个列表，列表里每个元素是消息字典
        return JsonResponse(data=all_message[::-1], safe=False)
